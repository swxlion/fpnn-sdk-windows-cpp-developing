...
namespace fpnn
{
	...
	private:
		...
		ConnectionInfo(SOCKET socket_, int port_, const std::string& ip_, bool isIPv4): _mutex(0),
		...
		void changeToUDP(SOCKET newSocket, uint8_t* socketAddress)	//-- UDP Client Used.
		{
			_isTCP = false;
			socket = newSocket;
