	...
	_requireCacheSendData(false), _embedRecvNotifyDeleagte(NULL)
{
	...
	if (host.find(':') == std::string::npos)
	{
		...
		{
			_isIPv4 = true;
			_connectionInfo.reset(new ConnectionInfo(INVALID_SOCKET, port, host, true));
		...
		else
		{
			...
			if (getIPAddress(host, IPAddress, eType))
			{
				...
				{
					_isIPv4 = true;
					_connectionInfo.reset(new ConnectionInfo(INVALID_SOCKET, port, IPAddress, true));
					_endpoint = std::string(IPAddress + ":").append(std::to_string(port));
				}
				else
				{
					_isIPv4 = false;
					_connectionInfo.reset(new ConnectionInfo(INVALID_SOCKET, port, IPAddress, false));
	...
	{
		_isIPv4 = false;
		_connectionInfo.reset(new ConnectionInfo(INVALID_SOCKET, port, host, false));
...
void Client::processQuest(FPQuestPtr quest, ConnectionInfoPtr connectionInfo)
{
	...
	catch (const FpnnError& ex)
	{
		...
		if (quest->isTwoWay())
		{
			if (_questProcessor->getQuestAnsweredStatus() == false)
				answer = FPAWriter::errorAnswer(quest, ex.code(), std::string(ex.what()) + ", " + connectionInfo->str());
		}
	}
	catch (const std::exception& ex)
	{
		...
		{
			if (_questProcessor->getQuestAnsweredStatus() == false)
				answer = FPAWriter::errorAnswer(quest, FPNN_EC_CORE_UNKNOWN_ERROR, std::string(ex.what()) + ", " + connectionInfo->str());
		}
	}
	catch (...){
		...
		{
			if (_questProcessor->getQuestAnsweredStatus() == false)
				answer = FPAWriter::errorAnswer(quest, FPNN_EC_CORE_UNKNOWN_ERROR, std::string("Unknown error when calling processQuest() function, ") + connectionInfo->str());
		}
	}
	...
	if (quest->isTwoWay())
	{
		if (questAnswered)
		{
			...
			answer = FPAWriter::errorAnswer(quest, FPNN_EC_CORE_UNKNOWN_ERROR, std::string("Twoway quest lose an answer. ") + connectionInfo->str());
	}
	else if (answer)
	{
	...
	if (answer)
	{
		...
		try
		{
			...
			FPAnswerPtr errAnswer = FPAWriter::errorAnswer(quest, ex.code(), std::string(ex.what()) + ", " + connectionInfo->str());
			raw = errAnswer->raw();
		}
		catch (const std::exception& ex)
		{
			FPAnswerPtr errAnswer = FPAWriter::errorAnswer(quest, FPNN_EC_CORE_UNKNOWN_ERROR, std::string(ex.what()) + ", " + connectionInfo->str());
			raw = errAnswer->raw();
		}
		catch (...)
		{
			...
			FPAnswerPtr errAnswer = FPAWriter::errorAnswer(quest, FPNN_EC_CORE_UNKNOWN_ERROR, std::string("exception while do answer raw, ") + connectionInfo->str());
			raw = errAnswer->raw();
		}
