...
bool ARQParser::processReliableAndMonitoredPackage(uint8_t type, uint8_t flag)
{
	...
	{
		/*
		 * Vistual Studio is unfriendly with Chinese comments without BOM header.
		 * I don't want to translate the Chinese comments here anymore. For related notes,
		 * please refer to the corresponding location of the Linux version.
		 * If the cases described in the Chinese comments, please mail to
		 * wangxing.shi@ilivedata.com, or swxlion@hotmail.com.
		*/
...
bool ARQParser::decodeBuffer(uint8_t* buffer, uint32_t len)
{
	...
	if (_parseResult->_embedInfos._embedRecvNotifyDeleagte == NULL)
	{
	...
	{
		_parseResult->_embedInfos._embedRecvNotifyDeleagte(_parseResult->_embedInfos.connectionId, buffHeader, currPackageLen);
	}
...
bool ARQParser::parseDATA()
{
	...
	else
	{
		...
		//-- For avoid the maliciously constructed invalid packets attacking, the following
		//-- codes will do the checking before allocing the memories.
		...
		//-- Prevent malicious construction of repeated packet attacking
