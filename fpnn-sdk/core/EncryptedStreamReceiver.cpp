...
LPWSABUF EncryptedStreamReceiver::beginRecv()
{
...
void* EncryptedStreamReceiver::transferRecvBuffer()
{
	void* buf;
	if (_curr < FPMessage::_HeaderLength)
	{
...
bool EncryptedStreamReceiver::receivedData(DWORD receivedBytes, FPQuestPtr& quest, FPAnswerPtr& answer)
{
	quest = nullptr;
	answer = nullptr;
	...
	if (_curr == _total)
	{
		...
		else
		{
			...
			_encryptor.decrypt((uint8_t *)buf + FPMessage::_HeaderLength, _bodyBuffer + FPMessage::_HeaderLength, _total - FPMessage::_HeaderLength);
...
bool EncryptedStreamReceiver::embed_receivedData(uint64_t connectionId, DWORD receivedBytes, EmbedRecvNotifyDelegate delegate)
{
	_curr += (int)receivedBytes;
	if (_curr == _total)
	{
